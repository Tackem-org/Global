...
func set(key string, value string) (bool, error) {
	...
	logging.Debug(debug.FUNCTIONARGS, "[FUNCTIONARGS] key=%s, value=%s", key, value)
...
func SetBool(key string, value bool) (bool, error) {
	...
	logging.Debug(debug.FUNCTIONARGS, "[FUNCTIONARGS] key=%s, value=%t", key, value)
...
func SetFloat64(key string, value float64) (bool, error) {
	...
	logging.Debug(debug.FUNCTIONARGS, "[FUNCTIONARGS] key=%s, value=%f", key, value)
...
func SetInt(key string, value int) (bool, error) {
	...
	logging.Debug(debug.FUNCTIONARGS, "[FUNCTIONARGS] key=%s, value=%d", key, value)
...
func SetIntSlice(key string, value []int) (bool, error) {
	...
	logging.Debug(debug.FUNCTIONARGS, "[FUNCTIONARGS] key=%s, value=%v", key, value)
	valuesText := []string{}
	var s string
	for _, i := range value {
		s = fmt.Sprintf("%d", i)
...
func SetTime(key string, value time.Time) (bool, error) {
	...
	logging.Debug(debug.FUNCTIONARGS, "[FUNCTIONARGS] key=%s, value=%s", key, value.Format("2006-01-02T15:04"))
	return set(key, strconv.FormatInt(value.Unix(), 10))
}

func SetDuration(key string, value time.Duration) (bool, error) {
	logging.Debug(debug.FUNCTIONCALLS, "[FUNCTIONCALL] Global.config.Set")
	logging.Debug(debug.FUNCTIONARGS, "[FUNCTIONARGS] key=%s, value=%s", key, helpers.DurationToString(value))

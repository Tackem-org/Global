test:
	@FILES="$(shell go list ./... | grep -v /pb/ )"; go test -timeout 30s -cover -coverprofile=cover.out $$FILES
	@go tool cover -func=cover.out | tail -n 1

test-debug:
	@FILES="$(shell go list ./... | grep -v /pb/)"; go test -timeout 30s -cover -v -coverprofile=cover.out $$FILES
